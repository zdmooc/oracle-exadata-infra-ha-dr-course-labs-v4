    ...
    Ce module permet de maîtriser **Consolidation de bases, séparation ressources, gouvernance, risques et modèles d’exploitation**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Consolidation** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **consolidation** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **multi-tenant** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **resource isolation** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **workload class** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **capacity planning** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **chargeback** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    La consolidation réunit plusieurs workloads sur la même Database Machine. Elle demande séparation logique, gouvernance I/O, CPU/mémoire, services RAC, fenêtres batch et politiques de monitoring.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Consolidation**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
...
select username,consumer_group from v$session where type=USER fetch first 30 rows only;
    ```
    ...
    |---|---|
    | sessions par service/PDB | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| CPU DB time par workload | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| I/O par base | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| fenêtres batch | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| incidents par tenant | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Une nouvelle base reporting doit être consolidée avec OLTP. L’équipe identifie ressources, fenêtres, services et exigences RPO/RTO pour éviter le noisy neighbor.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | consolider sans IORM ni Resource Manager | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| partager services critiques et batch | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| oublier sauvegarde et patching par tenant | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | classer workloads | À intégrer dans le livrable du module. |
| documenter SLA | À intégrer dans le livrable du module. |
| utiliser services et groupes ressources | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Élaborer une matrice de consolidation multi-workload avec risques et contrôles.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quand la consolidation devient-elle risquée ?
2. Quel rôle jouent services RAC et IORM ?

    ## Livrables attendus

    Matrice consolidation workload/SLA/contrôle.
