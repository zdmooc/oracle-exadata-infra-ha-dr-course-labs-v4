    ...
    Ce module permet de maîtriser **Sauvegarde et récupération Exadata : RMAN, FRA, archivelogs, ZDLRA, restore validation et Data Guard**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.
    ...
    |---|---|
    | **RMAN** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **FRA** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **archivelog** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **controlfile autobackup** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **recovery catalog** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **ZDLRA** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **restore validation** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **Data Guard** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
    ...
    La stratégie backup Exadata doit couvrir sauvegarde, restauration, archivelogs, catalogue, impact performance, réseau/FRA/ZDLRA et coordination avec Data Guard.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Backup and Recovery**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
...
select * from v$recovery_file_dest;
    ...
    |---|---|
    | dernière sauvegarde complète/incrémentale | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| archivelogs disponibles | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| FRA used percent | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| controlfile autobackup | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| temps de restore testé | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| RPO/RTO | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Audit recoverability avant changement. Vérifier mode archivelog, backups récents, FRA, catalog et preuve de restore test.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | déclarer sauvegarde OK sans test restore | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| oublier archivelogs | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| saturer FRA | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| confondre Data Guard et backup | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | tester restore régulièrement | À intégrer dans le livrable du module. |
| documenter RPO/RTO | À intégrer dans le livrable du module. |
| surveiller FRA et archivelogs | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Auditer read-only une stratégie RMAN et produire matrice RPO/RTO.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Pourquoi Data Guard ne remplace pas RMAN ?
2. Quelle preuve valide une stratégie de recovery ?

    ## Livrables attendus

    Rapport backup/recovery evidence.
