...
**Alignement Oracle University :** Extension infrastructure-first : ASM, diskgroups, redondance et layout..  
...
À la fin de ce module, l’apprenant doit pouvoir expliquer le périmètre **ASM et modèle de stockage**, le relier aux composants Exadata concernés, identifier les commandes de lecture utiles, formuler les risques principaux et produire un livrable d’exploitation vérifiable.
...
ASM constitue la couche de virtualisation du stockage Oracle Database. Sur Exadata, les grid disks exposés par les cellules alimentent des diskgroups qui portent DATA, RECO, DBFS ou autres usages selon le design. Le module insiste sur la lecture, la validation et les impacts de redondance sans proposer d’opérations destructives.
