    ...
    Ce module permet de maîtriser **Monitoring autres composants : PDU, ILOM, switches, disques, ventilateurs, alimentation et hardware**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Monitoring Other Components** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **ILOM** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **PDU** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **power** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **fans** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **switches** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **hardware alerts** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **environmental monitoring** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Les composants hors DB/cell incluent gestion matérielle, alimentation, ventilation, switches et accès out-of-band. Le monitoring doit être coordonné avec procédures datacenter.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Monitoring Other Components**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
    ...
    ```bash
    ipmitool sel list  # si autorisé et installé
ipmitool sensor  # lecture capteurs, privilèges requis
cellcli -e list alerthistory where alertMessage like %power% detail
cellcli -e list alerthistory where alertMessage like %fan% detail
    ```
    ...
    |---|---|
    | sensor status | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| power/fan alerts | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| switch link state | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| température capteurs | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| événements ILOM | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Une alerte alimentation apparaît. Collecter événements matériels, statut capteurs et alerthistory sans manipuler PDU ni redémarrer.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | ignorer alertes environnementales | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| intervenir physiquement sans support | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ne pas tracer numéro série/composant | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | escalader selon contrat support | À intégrer dans le livrable du module. |
| collecter serial et timestamp | À intégrer dans le livrable du module. |
| ne jamais masquer une alerte matérielle | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Préparer une fiche incident composant matériel Exadata.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quels composants ne sont pas visibles depuis la base ?
2. Pourquoi les logs matériels sont sensibles ?

    ## Livrables attendus

    Fiche monitoring autres composants.
