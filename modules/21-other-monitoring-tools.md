    ...
    Ce module permet de maîtriser **Outils complémentaires : Exachk, ORAchk, TFA, OSWatcher, AWR, ASH, logs et runbooks**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Other Monitoring Tools** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.
...
| **OSWatcher** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **AWR** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **ASH** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **log bundle** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **evidence correlation** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Ces outils complètent EM et commandes natives. Ils produisent des rapports santé, collectes diagnostic et historiques OS/DB utiles pour support et root cause analysis.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Other Monitoring Tools**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
    ...
    ```bash
    exachk -v
exachk  # collecte santé, lire documentation et fenêtre adaptée
tfactl print status
tfactl diagcollect -srdc <type>  # collecte potentiellement volumineuse, à cadrer
ls -ltr $ORACLE_BASE/diag
    ```
    ...
    |---|---|
    | score/risques Exachk | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| état TFA | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| taille bundles | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| alert log errors | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| AWR/ASH période incident | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Oracle Support demande des preuves. L’équipe prépare Exachk, TFA, AWR et logs pertinents en retirant secrets et données sensibles.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | envoyer logs sans revue sécurité | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| lancer diagcollect sans cadrer période | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| oublier AWR/ASH incident | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | limiter fenêtre temporelle | À intégrer dans le livrable du module. |
| masquer secrets | À intégrer dans le livrable du module. |
| nommer bundles par SR et timestamp | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Composer un pack d’évidence support avec liste de fichiers et justification.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quand utiliser TFA plutôt qu’Exachk ?
2. Quels risques de confidentialité existent ?

    ## Livrables attendus

    Checklist outils monitoring complémentaires.
