    ...
    Ce module permet de maîtriser **Introduction au workshop, objectifs, périmètre, posture read-only et organisation du cours**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Introduction** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **périmètre pédagogique du workshop** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **séparation théorie/lab/runbook** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **principe de non-destruction** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **rôle DBA, infra et support** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Le module positionne les couches DB servers, storage cells, ASM, Grid Infrastructure, réseau privé et outils support afin que l’apprenant comprenne où chaque preuve doit être collectée.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Introduction**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
    ...
    ```bash
    git grep -n "Exadata" modules docs labs
find modules labs templates scripts diagrams -maxdepth 1 -type f | sort
bash scripts/completeness-check.sh
    ...
    |---|---|
    | couverture modules/labs/templates | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| présence de livrables | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| traçabilité des références | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Un responsable formation veut savoir si le support couvre administration, monitoring, patching et cloud sans mélanger le dépôt tiers. La preuve est le mapping, le rapport final et l’inventaire Git.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | confondre Exadata avec une simple base RAC | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| présenter des commandes de modification dans un cours d’observation | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ne pas distinguer on-prem et Cloud@Customer | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | annoncer les limites de lab sans vraie machine | À intégrer dans le livrable du module. |
| lier chaque lab à un livrable | À intégrer dans le livrable du module. |
| citer le dépôt tiers uniquement comme complément | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Auditer le dépôt, vérifier les 28 modules, ouvrir le mapping officiel et produire une note de prise en main.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Pourquoi le cours privilégie-t-il les commandes de lecture ?
2. Quel est le rôle du dépôt tiers complémentaire ?

    ## Livrables attendus

    Fiche de cadrage du workshop et checklist de navigation.
