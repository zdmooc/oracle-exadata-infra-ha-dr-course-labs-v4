...
**Alignement Oracle University :** Key Capabilities of Exadata Database Machine.  
...
À la fin de ce module, l’apprenant doit pouvoir expliquer le périmètre **Key Capabilities**, le relier aux composants Exadata concernés, identifier les commandes de lecture utiles, formuler les risques principaux et produire un livrable d’exploitation vérifiable.
...
Les capacités clés d’Exadata incluent l’offload SQL vers les cellules de stockage, l’usage intelligent du flash, la priorisation I/O via IORM, la compression Hybrid Columnar Compression, la résilience ASM, la supervision intégrée et les outils d’exploitation fournis par Oracle. Ces fonctions doivent être comprises comme un ensemble cohérent, et non comme des options isolées [1] [3].
