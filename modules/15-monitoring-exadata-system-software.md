    ...
    Ce module permet de maîtriser **Monitoring Exadata System Software, versions, imageinfo, alertes, AHF et état logiciel**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Monitoring Exadata System Software** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **Exadata System Software** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **imageinfo** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **imagehistory** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **versions** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **AHF health** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **software drift** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Le software Exadata s’observe sur DB nodes et cells. Les versions, images, historiques et outils AHF permettent d’identifier dérives et prérequis support.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Monitoring Exadata System Software**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
    ...
    ```bash
    imageinfo  # privilèges selon hôte
imagehistory  # privilèges selon hôte
cellcli -e list cell attributes releaseVersion,kernelVersion
ahfctl status
    ```
    ...
    |---|---|
    | releaseVersion cell | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| image version DB node | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| historique patch | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| état AHF | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| écarts de versions | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Avant patching, l’équipe compare versions DB nodes/cells, historique image et état AHF pour préparer le pré-check.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | mélanger version DB et image Exadata | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ne pas archiver imagehistory | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ignorer AHF obsolète | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | inventaire versions avant chaque changement | À intégrer dans le livrable du module. |
| conserver preuves dans CAB | À intégrer dans le livrable du module. |
| ne pas patcher sans note Oracle applicable | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Construire un tableau versions Exadata System Software par composant.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quelle différence entre version Oracle Database et image Exadata ?
2. Pourquoi imagehistory est utile ?

    ## Livrables attendus

    Inventaire software Exadata.
