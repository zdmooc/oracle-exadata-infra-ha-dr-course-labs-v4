    ...
    Ce module permet de maîtriser **Offload SQL, predicate filtering, projection colonnes, Storage Indexes, HCC et diagnostics**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Smart Scan** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.
...
| **predicate filtering** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **column projection** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **direct path read** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
...
| **HCC** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **cell smart table scan** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Smart Scan déporte certaines opérations vers les storage cells lorsque le chemin d’accès, le type de segment et les prédicats le permettent. Le résultat attendu est moins de données renvoyées vers les DB servers.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Smart Scan**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
...
select sql_id,cell_offload_eligible_bytes,cell_offload_returned_bytes,physical_read_bytes from v$sql where sql_id=&SQL_ID;
    ...
    |---|---|
    | cell_offload_eligible_bytes | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| cell_offload_returned_bytes | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
...
| direct path read events | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Smart Scan absent sur une requête supposée éligible. On vérifie plan, méthode d’accès, direct path, statistiques offload et contraintes fonctionnelles avant toute modification.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | attendre Smart Scan sur index lookup très sélectif | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| oublier projections colonnes | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| forcer des paramètres cachés | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | diagnostiquer avec DBMS_XPLAN et v$sql | À intégrer dans le livrable du module. |
| mesurer eligible vs returned | À intégrer dans le livrable du module. |
| documenter pourquoi Smart Scan ne s’applique pas | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Diagnostiquer l’absence de Smart Scan pour un SQL_ID en collectant plan, stats offload et interprétation prudente.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quel ratio comparer pour évaluer l’offload ?
2. Pourquoi direct path est souvent lié à Smart Scan ?

    ## Livrables attendus

    Fiche diagnostic Smart Scan not used.
