    ...
    Ce module permet de maîtriser **Monitoring DB servers, GI, ASM, RAC, services, ressources OS et bases de données**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Monitoring DB Servers** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **DB server health** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **Grid Infrastructure** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **ASM** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **RAC services** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **OS resources** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **database role** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Les DB servers portent Oracle Database, GI, ASM, services RAC, OS et agents. Leur monitoring combine CRS, srvctl, ASM, OS et vues database.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Monitoring Database Servers**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
...
| services non démarrés | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| ASM free/usable | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| CPU load contextuelle | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| sessions par instance | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** Un service applicatif répond lentement sur une instance. Vérifier placement service, état RAC, ASM et événements DB avant action.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | redémarrer un service sans comprendre placement | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ignorer ASM | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ne pas vérifier gv$instance | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | conserver état CRS avant incident | À intégrer dans le livrable du module. |
| documenter services critiques | À intégrer dans le livrable du module. |
| croiser DB et OS | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Produire un DB server healthcheck read-only complet.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Pourquoi srvctl et crsctl sont complémentaires ?
2. Quel lien entre ASM et disponibilité DB ?

    ## Livrables attendus

    Rapport DB server healthcheck.
