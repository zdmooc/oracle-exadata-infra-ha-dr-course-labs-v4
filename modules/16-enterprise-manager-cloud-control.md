    ...
    Ce module permet de maîtriser **Enterprise Manager Cloud Control, targets Exadata, agents, incidents, métriques et tableaux de bord**. À la fin du module, l’apprenant doit savoir expliquer le sujet, identifier les composants Exadata concernés, collecter des preuves en lecture seule et produire un livrable exploitable par une équipe DBA, infrastructure, support ou cloud.

    ## Alignement avec le cours officiel

    Ce module couvre le thème Oracle University **Enterprise Manager Cloud Control** du parcours **Exadata Database Machine Administration Workshop**. Il enrichit le thème avec une approche infrastructure-first, des commandes non destructives, des métriques observables et un mini-lab ciblé.

    ## Concepts clés

    | Concept | Explication opérationnelle |
    |---|---|
    | **Enterprise Manager target** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **agent** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **incident rule** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **metric extension** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **blackout** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |
| **dashboard** | Élément à maîtriser dans ce module ; l’apprenant doit savoir le reconnaître, le relier à la couche Exadata concernée et produire une preuve observable. |

    ## Architecture concernée

    Enterprise Manager représente DB servers, databases, ASM, cells et Exadata rack comme cibles corrélées. L’agent et les plugins doivent être surveillés comme partie de la chaîne d’observabilité.
    ...
    Pour ce module, l’analyse doit rester spécifique à **Enterprise Manager Cloud Control**. Les preuves attendues ne sont pas un simple inventaire système ; elles doivent démontrer la compréhension du mécanisme Exadata concerné, de ses dépendances et de ses limites.
    ...
    ```bash
    emctl status agent
emctl upload agent  # opération d’upload, à utiliser seulement si autorisé
...
emctl config agent listtargets
    ...
    |---|---|
    | agent up/down | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| targets discovered | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| incidents ouverts | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| metric collection errors | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |
| blackouts actifs | À comparer avec la période, le workload et la couche concernée ; aucun seuil universel n’est inventé dans ce support. |

    ## Exemple d’analyse

    **Symptôme.** EM ne remonte plus les alertes cells. On vérifie agent, targets, plugins et blackouts avant de conclure à l’absence d’incident.
    ...
    **Prochaine action prudente.** Si l’action dépasse la lecture, produire un runbook, obtenir validation CAB ou support, et documenter rollback, impact et communication.
    ...
    |---|---|
    | penser qu’absence d’alerte EM signifie absence de problème | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| oublier blackout maintenance | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |
| ne pas surveiller l’agent | Produire une preuve, relire le runbook et escalader si l’action dépasse la lecture. |

    ## Bonnes pratiques

    | Bonne pratique | Mise en œuvre |
    |---|---|
    | définir tableaux de bord par rôle | À intégrer dans le livrable du module. |
| tester alerting | À intégrer dans le livrable du module. |
| réconcilier EM avec CellCLI/AHF | À intégrer dans le livrable du module. |

    ## Mini-lab ciblé

    **Objectif.** Établir une checklist EM pour rack Exadata et identifier les cibles critiques.
    ...
    **Résultat attendu.** Le résultat doit relier un symptôme ou un objectif pédagogique à des preuves Exadata spécifiques, et non à un simple inventaire générique.
    ...
    1. Quels targets EM sont nécessaires ?
2. Pourquoi vérifier les blackouts ?

    ## Livrables attendus

    Checklist Enterprise Manager.
