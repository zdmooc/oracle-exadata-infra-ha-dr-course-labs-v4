# Lab 18 — Cloud Service et Cloud@Customer operations

## Objectif

Ce lab vise à comparer responsabilités et opérations selon modèle Exadata. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-18-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
