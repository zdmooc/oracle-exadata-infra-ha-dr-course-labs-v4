# Lab 13 — Enterprise Manager readiness

## Objectif

Ce lab vise à vérifier cibles, agents, seuils, tableaux de bord et alertes. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-13-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
