# Lab 17 — Automated Support Ecosystem

## Objectif

Ce lab vise à préparer dossier AHF/Exachk/ASR/SR support. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-17-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
