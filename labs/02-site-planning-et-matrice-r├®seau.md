...
Ce lab vise à préparer IP plan, DNS, NTP, VLAN, flux et responsabilités. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-02-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
