# Lab 06 — Smart Scan validation

## Objectif

Ce lab vise à observer plans, statistiques et métriques d’offload sans forcer la production. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-06-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
