# Lab 05 — ASM et diskgroups

## Objectif

Ce lab vise à relier grid disks, diskgroups ASM et usages DATA/RECO/DBFS. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-05-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
