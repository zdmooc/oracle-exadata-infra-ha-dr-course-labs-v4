# Lab 03 — OEDA/OECA readiness review

## Objectif

Ce lab vise à relire les décisions de configuration initiale et les hypothèses. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-03-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
