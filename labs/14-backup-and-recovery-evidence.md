# Lab 14 — Backup and recovery evidence

## Objectif

Ce lab vise à auditer configuration RMAN, FRA, sauvegardes et restaurabilité. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-14-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
