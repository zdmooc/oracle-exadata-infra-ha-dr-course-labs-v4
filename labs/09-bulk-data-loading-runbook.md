# Lab 09 — Bulk Data Loading runbook

## Objectif

Ce lab vise à concevoir un chargement massif contrôlé avec DBFS ou alternatives. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-09-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
