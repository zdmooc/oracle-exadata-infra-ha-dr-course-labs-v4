# Lab 04 — CellCLI storage objects

## Objectif

Ce lab vise à lire cellules, cell disks, grid disks, flash et alertes. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-04-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
