# Lab 15 — HA/DR Data Guard switchover readiness

## Objectif

Ce lab vise à préparer une revue de bascule sans exécuter de failover. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-15-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
