# Lab 10 — Monitoring platform overview

## Objectif

Ce lab vise à construire une vue de supervision plateforme. Il est conçu pour être exécuté en priorité en environnement de formation ou de préproduction. Les commandes sont indicatives et doivent être adaptées à votre version et à vos droits.
...
```bash
mkdir -p evidence/lab-10-$(date +%Y%m%d-%H%M%S)
hostname -f
date
uname -a
ip -br addr
